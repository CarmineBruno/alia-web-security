...
function App() {
    const emptyProfile = { editing: false, name: "Philippe De Ryck", bio: "Teaching web security!", web: "https://pragmaticwebsecurity.com", video: "https://www.youtube-nocookie.com/embed/NPgVh5ozuZE" };
    ...
    return (
        <div className="App">
            <h1>User profile</h1>
            <button onClick={() => dispatch({type: "toggleEditing", payload: profile})}>{profile.editing ? "View" : "Edit"}</button>
            {!profile.editing && <button onClick={() => dispatch({type: "clearProfile"})}>Reset profile</button>}
...
function ProfileDisplay({ profile }) {
    return (
        <div>
            ...
            <h3>Keynote video</h3>
            <iframe title="website" src={profile.video} width="450" height="300"></iframe>
        </div>
    );
}
...
function ProfileEdit({ profile }) {
    ...
    return (
        <div>
            <h3>Name</h3>
            <p><input type="text" size="60" name="name" value={_profile.name} onChange={updateField} /></p>
            
            <h3>Bio</h3>
            <p><textarea name="bio" style={{fontFamily: "inherit"}} cols="60" rows="5" value={_profile.bio} onChange={updateField}></textarea></p>

            <h3>Website</h3>
            <p><input type="text" size="60" name="web" value={_profile.web} onChange={updateField} /></p>

            <h3>Keynote video</h3>
            <p><input type="text" size="60" name="video" value={_profile.video} onChange={updateField} /></p>
            <br />
...
function getYoutubeVideo(videoId) {
  ...
    // Run a sanity check on the full URL to make sure the URL is safe
